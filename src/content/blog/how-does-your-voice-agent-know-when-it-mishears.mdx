...
subtitle: "Evaluating voice agents on degraded audio, and why ASR confidence isn't enough"
description: "Most voice agents gate escalation on ASR confidence. Using a benchmark for capturing IDs over degraded audio, I show why that signal isn't enough, and what to use instead."
ogImage: "/images/og/voice-agent-mishears.png"
date: 2026-06-18
draft: false
---
...
Imagine a medical scribe agent capturing the wrong medication name, an emergency dispatch agent capturing the wrong street address, or an air traffic control agent hearing the wrong flight number. These errors can result in harm to humans.
 ...
 So the practical question, for anyone building one of these agents, is: how does it know?

This post builds a benchmark for capturing critical IDs over degraded, phone-like audio, and uses it to catch a regression: swapping in a bigger Whisper speech-to-text (ASR) model raised capture accuracy but quietly pushed the wrong-commit rate on impossible audio from 0% to 64%. From there I dig into why ASR confidence is a leaky proxy for correctness and reason toward better escalation signals.

Takeaways if you're building voice agents for noisy, high-stakes settings:

- **ASR confidence ≠ correctness.** Don't gate escalation on confidence alone. On degraded audio, Whisper will hallucinate a fluent transcript with high confidence.
- **Add cheap, independent checks.** Let the LLM sanity-check the transcript against the value it expects (a med name, an ID format); validate against a database or checksum; ask for a repeat and compare. Most wrong commits fall to one of these.
- **Don't trust the aggregate.** A tolerable-looking 24% overall wrong-commit rate hid a catastrophic 64% on the hardest audio. Slice your metrics by the conditions that match your production distribution, and include adversarial "impossible" cases that should trigger escalation.
- **Recalibrate per model, but only against your own data.** Thresholds optimal for one model (or one audio mix) aren't optimal for another, and F1 probably isn't your real objective. Tune on logged production audio against the actual cost of a wrong commit vs. a human review.

</Callout>

## The trouble with ASR Confidence

The obvious place to look for a signal for whether the model heard correctly is the ASR's own confidence. ASR API's typically hand you some version of per-token probabilities of the transcript they decoded. A standard way to collapse those into a single number is `exp(avg_logprob)`, the geometric mean of the per-token probabilities[^2]. Then you can wire it into an escalation policy: above some threshold, commit; below another, escalate; in between, ask the person to repeat.

So the signal is almost always *available*, but two things get in the way. First, note that the ASR model isn't the 'agent'. The ASR turns audio into text plus that confidence number. The agent is the ASR plus everything downstream that decides how to deal with that transcription. In many voice agents in production today that feature a typical ASR → LLM → TTS cascade, only the transcript *text* is passed to the LLM, so the agent never even sees the confidence and can't reason about how well it heard.

Second, even when you do feed that number into your escalation policy, it's a leaky proxy for correctness: it scores how confidently Whisper produced *the tokens it chose*, not whether those tokens are *true*. On clean-ish audio that's fine because a dip in confidence usually means a mistaken letter or two. But on degraded audio confidence decorrelates from correctness, and the agent can't tell from the number alone which regime it's in. Given severely degraded audio, Whisper will often hallucinate a transcription *with high confidence* (see the top image). So is it confident because it's correct, or because it confidently made something up?

None of this is news to the speech-recognition community: that raw decoder probabilities are poorly calibrated has been known for years. What's maybe less appreciated by people *building voice agents* is how unreliably this baseline signal can perform in a high-stakes escalation policy on degraded audio. 

In the rest of this post, we'll look at how Whisper ASR models behave on my ID capture benchmark to get a sense for this failure mode of ASR confidence as a predictor of speech-to-text transcription correctness. 
...
<Figure
  ...
  alt="Image showing next-token prediction in Whisper. On the clean audio (top) it makes decisive choices. On the severely distorted audio (bottom) it is much less certain."
  caption="Next-token prediction in Whisper. On the clean audio (top) it makes decisive choices. On the severely distorted audio (bottom) it is much less certain."
/>
...
- Code: [github.com/jackharrisonmohr/voice-agent-eval](https://github.com/jackharrisonmohr/voice-agent-eval)
