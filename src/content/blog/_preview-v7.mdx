...
This graph gives an intuition for how ASR confidence behaves differently based on model size and distortion level.

You can see these two regimes (severe vs non-severe) disentangled in the two mini-graphs above. In the non-severe audio graph (right), ASR confidence is a pretty good predictor of correctness. If you inspect the lower confidence dots you see examples where the model had mis-transcribed a few characters but got the general gist of the ID. In the severely distorted cases (left), all models are wrong 100% of the time. In the latter case, sometimes the model's confidence appears to be propped up by a hallucinated sentence. Other times, it hears a few characters correctly, but mostly misses it. 
